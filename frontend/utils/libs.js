...
export function addClassToCodeTags(htmlString) {
  // 正则表达式匹配<p>标签下的所有<code>标签
  const regex = /<p>([\s\S]*?)<\/p>/g;
  let result = htmlString.replace(regex, (match, pTagContent) => {
    // 正则表达式匹配<code>标签并添加class="abc"属性
    const codeRegex = /<code>/g;
    let newPTagContent = pTagContent.replace(codeRegex, '<code class="p-code">');
    return `<p>${newPTagContent}</p>`;
  });
  return result;
}
